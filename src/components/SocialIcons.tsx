...
/** Accepts https/http and relative "/" hrefs; rejects javascript: and data: URIs. */
export function safeSocialHref(url: string): string {
  try {
...
// Keys are lowercase — lookup is done with s.title.toLowerCase() so CDS
// capitalisation ("Facebook", "facebook", "X (Twitter)") all resolve correctly.
const SOCIAL_ICONS_MAP: Record<string, ReactElement> = {
  facebook: (
  ...
  twitter: (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"/></svg>
  ),
  // "x (twitter)" and "x" both map to the same X/Twitter icon
  "x (twitter)": (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"/></svg>
  ),
  x: (
...
/** Case-insensitive lookup — returns the SVG or undefined. */
